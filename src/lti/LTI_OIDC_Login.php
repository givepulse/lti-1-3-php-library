...
class LTI_OIDC_Login
{
     ...
     */
    public function do_oidc_login_redirect($launch_url, array $request = null): Redirect
    {
        if ($request === null) {
            $request = $_REQUEST;
        }
    ...
    protected function validate_oidc_login($request)
    {
        ...
        $registration = $this->db->find_registration_by_issuer_client_id($request['iss'], $request['client_id']);
